...
import { Calendar, Clock, Video, CheckSquare, Phone, Loader2, Link2, AlertTriangle, Trash2 } from 'lucide-react';
import { useCreateLeadTask, useUpdateLeadTask, useDeleteLeadTask, LeadTask } from '@/hooks/useLeadTasks';
...
export function EventModal({ open, onClose, task }: EventModalProps) {
  ...
  useEffect(() => {
    const check = async () => {
      if (!assignedTo || !scheduledDate) { setConflicts([]); return; }
      ...
      const overlapping = data.filter(t => {
        if (isEditing && t.id === task.id) return false;
        if (!t.scheduled_at) return false;
        const tStart = new Date(t.scheduled_at);
        const tEnd = new Date(tStart.getTime() + (t.duration_minutes || 60) * 60 * 1000);
  ...
  const resetForm = () => {
    setType('task'); setTitle(''); setDescription(''); setPriority('medium');
    setMeetLink(''); setScheduledDate(''); setScheduledTime('');
    setDuration('60'); setAssignedTo(''); setLeadId('');
  };

  const handleSubmit = async () => {
    if (!title.trim()) { toast.error('Título é obrigatório'); return; }
    if (!leadId) { toast.error('Selecione um lead'); return; }
    ...
    try {
      ...
      if (isEditing) {
        await updateTask.mutateAsync({
          ...
          priority,
          meet_link: type === 'meeting' && meetLink.trim() ? meetLink.trim() : null,
  ...
  const handleDelete = async () => {
    if (!task) return;
    try {
      await deleteTask.mutateAsync({ id: task.id, leadId: task.lead_id });
      ...
      <DialogContent className="sm:max-w-xl">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2 space-y-1.5">
            ...
            <Input value={title} onChange={e => setTitle(e.target.value)} placeholder="Ex: Reunião de apresentação" />
          </div>
          ...
          {type === 'meeting' && (
            <div className="col-span-2 space-y-1.5">
              ...
              <Input value={meetLink} onChange={e => setMeetLink(e.target.value)} placeholder="https://meet.google.com/xxx-xxx-xxx" />
            </div>
          )}
          ...
          <div className="col-span-2 space-y-1.5">
            ...
            <Textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Observações..." className="min-h-[80px]" />
          </div>
        </div>
